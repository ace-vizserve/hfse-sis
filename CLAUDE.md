...
**Committed on `main`, NOT yet pushed (2026-08-01→03; migrations 107–110 all applied to prod).** Triage of the 2026-07-31 academics training produced **KD #174/#175/#176** (three pre-existing defects: a config save that resized grading denominators without recomputing the grades on printed report cards — recompute moved to TS in `lib/grading/recompute-sheet.ts`, 107 returns `sheet_ids`, 108 stops the RPC writing `qa_total` so **SQL owns array shape, TS owns values**; a `SECURITY DEFINER` RPC missed by BOTH KD #167 revoke lists, now revoked and verified live at `42501`; and the Classroom roster linking every name to registrar-only `/records`). Four training features then shipped as **KD #177/#178/#179**: the excused-absence note + the "an excused absence must say why" rule on both marking surfaces (109); house on `public.students` (110, four **placeholder** rows); whole-year direction on the Records academic tab; at-risk widened to the three components plus a within-sheet outlier. Gate green each time — 2,433 tests, typecheck, build, prettier.

**Both house and at-risk were reworked on 2026-08-03 after a browser pass** — the house setter moved twice before landing as the **fourth tile in the permanent record's fact band**, and the at-risk **Alerts column was deleted** for a `Look up student` dialog: re-reading the transcript showed Koh asked to "flag out students", i.e. a list, not a per-row chip. KD #178/#179 amended. **Browser pass done (2026-08-04): all four confirmed by Mr Ace** — both marking surfaces, the house tile, the grade lookup, the Trend column. Not yet exercised: running the students sync and confirming a house survives it. **Koh's FCA half is unbuilt** — a form adviser has no grading sheet. **Grade data:** `scripts/audit-grade-recompute-drift.ts` (read-only) reports **AY2026 CLEAN** — 260 sheets, 4,636 entries; the one drifted sheet (SCI · Discipline 1 · T2) came from the backfill import and its **grades were right while its totals were wrong**, so it was repaired by restoring totals, not recomputing. AY2025 has a residual **58 entries across 50 sheets** (closed year, all locked) — undecided.
