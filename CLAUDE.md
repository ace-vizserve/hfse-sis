...
**Current state (2026-06-05):** Sprint 56 — **dashboard scope correctness + seeder fidelity**, merged to `main` and **pushed to prod**. Fixed a class of count-vs-drill mismatches: the Markbook **"Grades entered"** card + drill now both scope to the picker's term via a shared `termIdsForRange` (`lib/markbook/term-range.ts`); the dashboard **date picker no longer defaults to Term 1** — `getDashboardWindows` reuses the date-based `resolveCurrentTerm` (today → `is_current` → most-recently-ended → earliest, KD #116) so every module's `thisTerm` lands on the current/most-recent term; and **"Change requests pending"** is now a live **un-windowed** count (pending = open work needing action, not a windowed event). Plus a verified **cross-module seeder-fidelity pass** — `grade_entries.updated_at` term-spread, P-Files expiry + `seedPFileRevisions`, movements audit-context, evaluation realism, publication selectivity (three audit "criticals" were false positives — `annual_letter_grade`/calendar/late-enrollees already faithful — left alone). KD #124/#125. No migration. (Preceded same window by Sprint 54/55 — masterfile dashboard + Excel export + audit humanizer + email one-click approve/reject, KD #121–123.)
...
- Sprints 28–49 (2026-04-29 → 2026-05-31): see development-plan.md. Highlights: KDs #63–116 + migrations 030–068; Sprint 44 = Markbook polish + SOW teardown (062–066); Sprint 45 = Records student detail (KDs #111–112, migration 067); Sprint 46 = edge-case hardening + attendance proration (KD #113, migration 068); Sprint 49 = Attendance Daily view + shared date-derived current-term resolver `lib/sis/current-term.ts` (KD #116).
