...
**Current state (2026-06-02):** Sprint 51 — AY application window relocated to SIS Admin (KD #118, restoring the KD #48 boundary). `academic_years.accepting_applications` is now driven from `/sis/ay-setup`: a descriptive per-row `Switch` opens/closes each year's window; opening a non-current year is single-select (one early-bird AY at a time, via `lib/sis/early-bird.ts::computeEarlyBirdClosures`); switching the active AY auto-opens its window and closes the outgoing one; the AY Readiness pill turns amber "Setup needed" when the new current AY is under-configured. `/admissions/upcoming/applications` is now read-only. (An earlier same-day pass had put the control inside Admissions; reverted per KD #48.) No migration. Build clean, 112 tests. Preceded by Sprint 50 — late-enrollee position-aware decision (KD #117).
