...
**Both house and at-risk were reworked on 2026-08-03 after a browser pass** — the house setter moved twice before landing as the **fourth tile in the permanent record's fact band**, and the at-risk **Alerts column was deleted** for a `Look up student` dialog: re-reading the transcript showed Koh asked to "flag out students", i.e. a list, not a per-row chip. KD #178/#179 amended. ⚠ **Browser pass covers attendance + the house tile; the grade lookup and the Trend column are unseen.** For house, the check that matters is running the students sync and confirming it survives. **Koh's FCA half is unbuilt** — a form adviser has no grading sheet. **Grade data:** `scripts/audit-grade-recompute-drift.ts` (read-only) reports **AY2026 CLEAN** — 260 sheets, 4,636 entries; the one drifted sheet (SCI · Discipline 1 · T2) came from the backfill import and its **grades were right while its totals were wrong**, so it was repaired by restoring totals, not recomputing. AY2025 has a residual **58 entries across 50 sheets** (closed year, all locked) — undecided.
