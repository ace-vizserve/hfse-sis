...
**On prod, pushed (2026-08-01→04; migrations 107–110 applied).** Triage of the 2026-07-31 academics training produced **KD #174/#175/#176**, three pre-existing defects, in that order: **#174** the Classroom roster linking every name to registrar-only `/records`; **#175** a `SECURITY DEFINER` RPC missed by BOTH KD #167 revoke lists, now revoked and verified live at `42501`; **#176** a config save that resized grading denominators without recomputing the grades on printed report cards — recompute moved to TS in `lib/grading/recompute-sheet.ts`, 107 returns `sheet_ids`, 108 stops the RPC writing `qa_total` so **SQL owns array shape, TS owns values**. Four training features shipped as **KD #177/#178/#179**: the excused-absence note + the "must say why" rule on both marking surfaces (109); house on `public.students` (110, four **placeholder** rows); whole-year direction on the Records academic tab; at-risk widened to the three components plus a within-sheet outlier. House and at-risk were both reworked 2026-08-03 after a browser pass (house → fourth tile in the permanent record's fact band; the at-risk Alerts **column** deleted for a `Look up student` dialog, because Koh asked to "flag out students" = a list). **Browser pass done 2026-08-04, all four confirmed by Mr Ace.** Not yet exercised: a students sync surviving a house. **Koh's FCA half is unbuilt** — a form adviser has no grading sheet. **Grade data:** `scripts/audit-grade-recompute-drift.ts` reports **AY2026 CLEAN** (260 sheets, 4,636 entries); AY2025 has a residual **58 entries across 50 sheets** (closed year, all locked) — undecided.

**Uncommitted work in progress (2026-08-05, no migration).** Two level fixes then a process correction. (1) `/records/level-mismatches` → **"Levels needing attention"**: the review queue never read `level_aliases`, so a mapped label sat in the list forever and the badge never decremented; and a second list now names levels that have students waiting but **no class** — a blocker that previously surfaced only inside a per-student dialog. (2) **KD #180 — enrolment (step 10) and class assignment (step 11) are separate steps**, per `docs/context/admission-process.md`: `section_id` is optional on the Enrolled flip, placement-roles-only when supplied, attendance starts at **placement**, and P-Files now chases unplaced students. **`Enrolled (Conditional)` means ALN (trial class + SPED), never "enrolled without a class"** — the old code comment saying otherwise is what caused the drift. **No browser pass on any of it.** Note the suite has ~3 files that time out under full-suite load (`role-permissions-guardrails`, `student-lookup-sheet`, `grading-workbook-secondary-t2`) and pass in isolation — pre-existing flakiness, not a regression.
