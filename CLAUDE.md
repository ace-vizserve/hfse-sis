...
**Current state (2026-06-05):** Sprint 55 — **one-click Approve/Reject from the grade-change-request email**, merged to `main` and **pushed to prod**. The email's Approve/Reject buttons are now signed, single-effect **action tokens** (HMAC over `{requestId, action, approverId}`, no expiry, new server-only secret `CHANGE_REQUEST_ACTION_SECRET`) that open a **prefetch-safe confirm page** (`/change-requests/act`, in `proxy.ts` PUBLIC_PATHS) and decide **as that approver without a login**; the confirm POST reuses the in-app decision logic, now extracted to `lib/change-requests/decide.ts` and shared by `PATCH /api/change-requests/[id]` + the token route — identical guards, incl. **secondary co-sign from email**, reject-needs-reason, optimistic concurrency, audit (`via: 'email_token'`). Graceful degrade to the old deep-links when the secret is unset. KD #123. No migration. (Preceded same day by Sprint 54 — masterfile tracking dashboard + Excel export + audit-log humanizer, KD #121/#122.)
