...
| `docs/context/16-attendance-module.md`      | Attendance module — sprint-ready contract + DDL + Excel-import flow (T1 reference received) |
...
│   ├── (records)/records/    ← Records module: dashboard (consolidated — 5 Records aggregators + 5 admissions widgets + activity feed), students list + detail (4 tabs), discount-codes, audit-log
│   ├── (sis)/sis/            ← SIS Admin hub: /sis (landing — 3 admin cards + superadmin health strip), /sis/ay-setup (wizard), /sis/admin/approvers (superadmin)
...
│   ├── sis/                  ← queries.ts (Records reads) + dashboard.ts (records aggregators: pipeline/documents/levels/expiring/activity) + health.ts (SIS hub system-health strip) + ay-setup/{queries,admissions-ddl}.ts + approvers/queries.ts
...
├── components/markbook/      ← grade-distribution-chart, sheet-progress-chart, change-request-panel, publication-coverage-chart, recent-markbook-activity (module-specific dashboard widgets)
...
47. **Attendance module is the sole writer of daily attendance (planned).** When the Attendance module lands (route group `/attendance/*`, sprint blocked on HFSE Excel — see `docs/context/16-attendance-module.md`), it owns every write path for daily attendance. Markbook, Records, and Parent are read-only consumers: Markbook reads the `attendance_records` rollup for report cards + renders a summary card on `/markbook/sections/[id]` with a "Mark attendance →" deep-link to `/attendance/[sectionId]`; Records grows a chronological Attendance tab on `/records/students/[enroleeNumber]`; Parent sees the rollup on the published report card. The existing `/markbook/sections/[id]/attendance` editable grid retires on migration per `15-markbook-module.md` §Planned migrations. Single audit prefix `attendance.*`, one cache tag, one rollup write path — same pattern as KD #31 (P-Files repository) and KD #37 (SIS is sole writer of `'Rejected'`).
