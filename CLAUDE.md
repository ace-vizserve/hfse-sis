...
**Current state (2026-06-05):** Sprint 54 — masterfile reframe + cross-cutting polish, merged to `main` and **pushed to prod** (~52 commits, incl. the school-calendar rework). **Masterfile** is now a **tracking dashboard** (At a glance → How they're doing → Worth a look — a status-tracking view, _not_ a readiness gate) instead of the flat grid, with the exact spreadsheet living only in a new **Excel report-book export**, plus **Teacher's Comments** (FCA write-ups) on the masterfile — all generated from the same `loadMasterfile` data, no schema change (KD #122). **Audit logs** are humanized across every module — plain action labels + readable `context` one-liners that prefer the student's name over raw IDs, via the pure `lib/audit/humanize.ts`; companion plain-English UI-copy sweep removed `studentNumber`/jargon from prose (KD #121). Also shipped: operational **school-calendar** rework (term-scoped Month/Week/Day/List views, weekends, per-level chips, today-default, revert-to-school-day, full audit trail — `lib/attendance/calendar-operational.ts`); **theme recolor** to HFSE brand (blue `#213098` + orange `#ED7622`, darker greens) in `app/globals.css`; **grading-sheet** colored WW/PT/QA group headers; attendance student-lookup rollup fix (proration-aware). No migration.
...
- Sprints 28–48 (2026-04-29 → 2026-05-30): see development-plan.md. Highlights: KDs #63–113 + migrations 030–068; Sprint 44 = Markbook polish + SOW teardown (062–066); Sprint 45 = Records student detail (KDs #111–112, migration 067); Sprint 46 = edge-case hardening + attendance proration (KD #113, migration 068).
