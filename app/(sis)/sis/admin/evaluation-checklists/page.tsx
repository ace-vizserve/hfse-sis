...
export default async function EvaluationChecklistsAdminPage({
  ...
  const { data: subjectsRaw } = await service
    .from('subjects')
    .select('id, code, name')
    .order('name');
  ...
  const { data: levelsRaw } = await service
    .from('levels')
    ...
    <PageShell>
      ...
      <header className="space-y-3">
        ...
        <p className="max-w-2xl text-[15px] leading-relaxed text-muted-foreground">
          One list per term × subject × level. Subject teachers tick these off per student during
          the term; PTC use only. Never flows to the report card.
        </p>
      </header>
