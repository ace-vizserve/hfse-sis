...
2. **shadcn primitives over custom wrappers.** Use `Card`, `Table`, `Field`, `Select`, `Tabs`, `Sheet`, `Dialog`, `DropdownMenu` directly. `Surface` / `PageHeader` / `PageShell` are legacy wrappers kept for backwards-compat — do not reach for them on new work. Migrate existing pages to primitives _when you touch them_, not in bulk.
...
| `AlertDialog` + siblings                                                                                                     | Confirm dialogs for destructive / irreversible actions — never `window.confirm` (KD #21)                    |
| `Badge`                                                                                                                      | Status pills, counts                                                                                        |
| `Breadcrumb` + siblings                                                                                                      | Hierarchical navigation trails (e.g. Academic Summary child routes)                                         |
| `Button`                                                                                                                     | Every clickable action — **default variant carries the Aurora Vault gradient, do not override**             |
| `Calendar`                                                                                                                   | Date grid — consumed via the `DatePicker` family below, not used bare                                       |
...
| `Collapsible` + `CollapsibleTrigger` + `CollapsibleContent`                                                                  | Inline expand/collapse disclosure                                                                           |
| `Command` + siblings                                                                                                         | cmdk-backed command palette + searchable pickers (Cmd+K palette, multi-select toolbars)                     |
| `DatePicker` + `DateRangePicker` + `DateTimePicker`                                                                          | **Every date input** — native `<input type="date">` / `datetime-local` are banned (KD #44)                  |
| `Dialog` + siblings                                                                                                          | Modal forms and focused tasks                                                                               |
...
| `Form` + `FormField` + `FormItem` + `FormControl` + `FormMessage`                                                            | RHF + zod submit-based forms (KD #20)                                                                       |
| `Input`                                                                                                                      | Text / number / email / password fields                                                                     |
| `Label`                                                                                                                      | Standalone control labels — prefer `Field` for form rows                                                    |
| `Popover` + `PopoverTrigger` + `PopoverContent`                                                                              | Anchored floating panels (facet filters, inline editors, module switcher)                                   |
| `RadioGroup` + `RadioGroupItem`                                                                                              | Mutually-exclusive option sets                                                                              |
| `ScrollArea`                                                                                                                 | Styled scroll containers                                                                                    |
...
5. **Last resort: custom markup.** Only when no combination of primitives works (e.g. the project-specific grading grid with Blank ≠ Zero semantics). Leave a comment explaining _why_ custom.
...
| `/sis/admin/admission-options`     | data table with bulk actions | `SisPageHeader` (Current / Upcoming / Past year badge) + §9.4 accent "Closed right now" panel + §8 toolbar (level `Toggle` chips, "Closed only" `Switch`, track `Tabs` with counts) + one `Card p-0` `Table` (sticky header; SIS-level group rows → parent-facing name rows → one row per class type, a `Switch` column per session) + sticky bottom bulk bar (session `Select`, outline Open / Close) + `DropdownMenu` → `Sheet` | Group rows show which SIS level each parent-facing name counts as; filters are client-side; empty AY → §7.6 card with "Copy from AYxxxx" |
