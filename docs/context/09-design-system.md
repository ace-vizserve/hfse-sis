...
| `/sis/admin/admission-options`     | list grouped by level      | `SisPageHeader` + 3 `HubStat`s + one level container `Card` per SIS level, rows with inline `Switch`es + `DropdownMenu` → `Sheet` | Grouping shows which SIS level each parent-facing name counts as; empty AY → §7.6 card with "Copy from AYxxxx" |
