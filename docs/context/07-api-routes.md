...
| `/api/sis/ay-setup/accepting-applications`   | PATCH             | school_admin+                     | Early-bird application window (KD #118); `program: 'vizschool'` flips VizSchool's own switch (migration 176, no single-select) |
...
| `/api/parent/v2/admission-options`     | GET    | **None** — public, IP rate limit only  | `?ay=AY2027` (omit → upcoming else current AY). OPEN level / class type / schedule rows from `admission_options` (migration 174). 404 unless current or accepting                                                                                                              |
| `/api/parent/v2/academic-years`        | GET    | **None** — public, IP rate limit only  | The portal's year picker: years open to HFSE (`accepting_applications`) or VizSchool (`vizschool_accepting_applications`, migration 176), oldest first, never `AY9…`. `{ years: [{ ayCode, isCurrent, hfseOpen, vizschoolOpen }] }`. Cached 300s under `parent-academic-years` |
| `/api/parent/v2/enrolled-students`     | GET    | Bearer, as above                       | Children with a LIVE ENROLMENT this AY. ⚠ Not `/students` — that one is gated on publication and correctly omits an enrolled child between publications (KD #195)                                                                                                              |
...
⚠ **`/api/parent/v2/admission-options` is the one route here with no Bearer token**, on purpose: the portal's public `/complete-enrolment/:token` page must build the same dropdowns, and the response is only the list of classes the school offers — no student, parent or count. Its gate is the YEAR: served only when `is_current` or `accepting_applications`, never `AY9…`. Response `{ ayCode, options: [{ levelLabel, levelCode, classTypeLabel, schedule, sortOrder }] }`, `schedule` in the portal's words ("Morning" / "Afternoon" / "Whole Day"). Server-cached 300s under `admission-options:<ay>`; no HTTP caching, since CORS reflects the Origin. `proxy.ts` excludes all of `/api`, so no session is needed to reach it.
