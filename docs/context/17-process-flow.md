...
| 10  | **Markbook sync**           | Automatic — cron-driven sync (KD #90, KD #154) populates `students` + `section_students`; gaps surface on the Records unsynced-students queue (`/records/unsynced`) for manual "Assign section" resolution | Markbook                                                   |
