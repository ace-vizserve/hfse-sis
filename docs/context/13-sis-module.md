...
> **Status:** 🔶 Phases 1–2 shipped (2026-04-17). Phase 3 spec finalized 2026-04-18 — see `docs/sprints/development-plan.md` §Sprint 10 Phase 3. The sections below describe the module's intent and cross-module contract; for up-to-date shipped scope read the sprint plan.
...
- **Mid-year section transfer goes through the dedicated route, not the class-stage PATCH** (KD #67). `POST /api/sis/students/[enroleeNumber]/transfer-section` runs an atomic withdraw-old-row + insert-new-row pair against `section_students` (Hard Rule #6) plus an admissions-side `classSection`/`classLevel`/`classStatus='Finished'` update. The class-stage PATCH rejects post-Enrolled `classSection` mutations with a 422 pointing to the new endpoint — closes the silent dual-section bug. Term of transfer captured in audit context via shared `lib/sis/terms.ts::getTermForDate`. UI: per-row "Move" action on the SIS Admin section roster table at `/sis/sections/[id]`.
- **Late-enrollee term detection** (KD #68). Tagging a student as `late_enrollee` via `<EnrolmentEditSheet>` refreshes `enrollment_date=today` on the active→late_enrollee boundary (boundary-only — idempotent re-saves don't re-stamp) so the joining-term lookup against `terms` reflects the registrar's tag moment. Same `getTermForDate` helper as transfer; audit context captures `lateEnrolleeTermLabel`. Records placement section's StatusBadge gets an inline "· T2" amber suffix for late-enrollee rows, derived from a preloaded terms-by-AY map.
...
- SMS / email sequences / outreach automation — later-phase CRM scope if that lane ever opens
