...
| `report_card_comments` | Per-student narrative comments | Module-owned |
...
| `/admin/sections` list + per-section roster CRUD (list view + student-in-section management — **NOT** the attendance or adviser-comments tabs on the same route) | **Records module** (Records) — or **Scheduling** once it exists | Who-is-in-which-class is a records/scheduling concern. The attendance and comments tabs rendered at the same route stay with Markbook because they're academic data; only the roster layer moves. |
