...
- **Approval.** Two people at the school approve each declaration — the child's
  form class adviser, then the officer in charge **for that child's half of the
  school** (Primary or Secondary). That is SIS-side and it is now built. Your
  job still ends at showing the status.
...
1. ~~Nobody holds the "officer in charge" post.~~ **Answered 2026-08-27** —
   there are **two officers, one for Primary and one for Secondary**, and a
   child goes to whichever matches their year. Both steps are live, so a
   declaration can now go all the way through. ⚠ **On the SIS side the routing
   between the two is still being fixed**, so treat approval timing as
   unpredictable for now. A parent may see "With the school" for a while:
   do not add copy promising a timeframe.
