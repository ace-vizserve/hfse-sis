...
## Status snapshot (last updated 2026-04-23, twentieth pass — Admissions module split from Records (`/admissions/*` with dedicated `admissions` role, module switcher derives allowed modules from `ROUTE_ACCESS`), cross-year Records via `studentNumber` through `lib/sis/records-history.ts`, Test environment via AY9999 Environment switcher at `/sis/admin/settings` (migrations 025 + 026; seeder fills grade entries, daily attendance + rollups, teacher_assignments, evaluation writeups, Enrolled admissions rows, pre-enrolment funnel, discount codes, publication window; destructive reset + `delete_academic_year` RPC), AY-admissions-table naming normalized to 4-digit `ay{YYYY}_*` (migration 026: RPC validators + wizard slug + env/seeder prefixes all aligned). 3 new KDs (#51 Admissions split, #52 Test env, #53 4-digit slug).)
