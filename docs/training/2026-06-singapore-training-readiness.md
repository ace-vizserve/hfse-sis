...
This is the system's **first real-usage event** — the first time HFSE staff touch it hands-on. The bar is **"training-ready," not "full production cutover."** Training _is_ the UAT: it surfaces the real bugs, workflow bottlenecks, and gaps. Sequence:
...
Run _exactly the flows you'll train_, end-to-end, in the training env, as each role. Fix any blocker before the 22nd.
...
Format suggestion: a short "do this → see this" task recipe per workflow + a screenshot. Keep each to the _happy path_; edge cases go in the champions pack.
...
3. **Network/access onsite** — confirm the prod/staging URL is reachable from the Singapore campus + accounts work _before_ the session (test from there if possible).
