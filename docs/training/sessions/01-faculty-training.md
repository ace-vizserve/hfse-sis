# Session 01 — Faculty / Teacher Training

**Audience:** subject teachers + form-class advisers · **Duration:** ~2.5–3 hrs · **Modules:** Markbook · Attendance · Evaluation
**Outcome:** every teacher can enter grades, mark attendance (incl. excused types), and write + submit an FCA comment — unaided.

**Before you start:** each trainee logged into their **training teacher account** (already assigned to a training section, so they see their own sheets/roster). Test-mode banner visible. Have the handout (Faculty pack) on each table.
...
4. **Quotas:** Vacation leave is 1/term, compassionate 5/year — if you exceed, you get a soft warning but it still records (the office handles exceptions).
5. **Daily view:** everyone defaults to **Present** — you only tap the exceptions (absent/late/excused). Fastest for daily marking. Change the date to load that day.
6. **Student lookup:** search a student to see their record without leaving the page.
7. **Late joiners:** cells before a student's enrolment date are dimmed — they're not counted against them.

**Hands-on:** "Mark a week on your term sheet — include one Absent, one Late, and one Excused (pick a kind). Then switch to Daily view and mark today."

**Competency check ✅:** trainee marks several days including one EX with the correct subtype, and can find a student via lookup.
...
**Competency check ✅:** trainee writes, saves as draft, and submits one FCA comment.
