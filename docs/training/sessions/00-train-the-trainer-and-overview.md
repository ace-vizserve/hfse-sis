...
1. **Concept (2–3 min)** — what this does + why it matters to _their_ day (frame in their words: "this is where you mark who's absent," not "the attendance writer").
2. **Live demo (5–8 min)** — you drive, projected. Narrate the click-path. Show the _happy path_ only.
...
Adjust to HFSE's actual room/time blocks — the _order_ matters more than the exact days (teachers first; champions after they've seen the modules; handover last).
...
- [ ] **Onsite access confirmed** — the URL loads + accounts log in _from the Singapore campus network_ (test ahead if possible).
...
4. Keep the energy on _their_ learning, not the bug.
