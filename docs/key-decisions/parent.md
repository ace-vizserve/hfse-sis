...
**Not built here, deliberately:** approval (**built** — KD #196), **the register write on final approval — still not built, and it is the next piece**; editing or withdrawing a filing; and notifying the parent of the outcome. ⚠ **Approving a filing marks nobody absent or excused today.** Mr Ace, 2026-08-27, on seeing the Decided tab fill: _"the attendance sheet is not showing that the filed student has been excused based on the approval details."_ Handoff brief for the portal's own repo: `docs/handoff/2026-08-27-parent-portal-declarations.md`.
