...
⚠ **SUPERSEDED IN PART, 2026-08-29 — the paragraph above describes the ROUTE's behaviour as it was, and the route no longer does this.** The 200 was answering a person the same way it answered a stuck button (see the section below). **What survives is the reasoning, not the mechanism:** the double-tap must still not be shown a failure, and it still is not — but the double-tap is now correctly identified as **two requests in flight at the same instant**, which the overlap check cannot see at all and which the unique index's `23505` branch catches. `isExactMatch` still exists and still distinguishes the two shapes for the portal's benefit; it no longer decides success-vs-failure.
...
✅ **FIXED 2026-08-29 — a re-filing is told where it got to, instead of being told it worked.**

A parent re-filing dates already on record got **200 with the OLD filing**. Nothing was written, so they were told it had worked and then could not find it. Mr Ace, who hit it filing 3 Sep twice: _"its pending for approval already, refiling for the same date and it succeeds is confusing"_.

`findOverlappingFilings` now selects `status` and carries it on `OverlappingFiling`; the exact-match 200 branch is gone and **both shapes answer 409**, worded by state — pending says the school has not decided yet, approved says it is settled and to contact the office. `alreadyFiledMessage` holds both sentences.

⚠ **THE DOUBLE-TAP PROTECTION IS NARROWED, NOT REVERSED, and this is the thing to carry.** The genuine double-tap is **two requests in flight at the same instant**. Neither can see the other in the overlap check — they both pass it and race to the insert — so it was never this branch that protected them. The unique index's `23505` branch does, and still answers that race with a 200. A re-send that arrives _after_ the first has landed is a person, and a person is told. **Do not "restore" the 200 in the overlap branch.**

⚠ **`rejected` and `cancelled` needed NO work and none was done.** Migration 130 had already removed `rejected` from the duplicate index, and `findOverlappingFilings` never counted either status, so re-filing after the school asks for a certificate already worked. It was **verified, not built**.

**Proven against production** by `scripts/verify-refile-blocked.ts`: a real approved absence, re-filed as the real filing parent through the real route, returned 409 with the approved wording, `overlapping[0].status` survived the live SELECT, and the `student_declarations` row count did not move. The script counts either side precisely so the test cannot silently create the duplicate it exists to prevent.

🔴 **THE ORIGINAL WRITE-UP OF THIS BUG WAS WRONG TWICE, AND BOTH ERRORS CAME FROM REASONING INSTEAD OF READING.** Recorded because the failure mode will recur. **(1) The "silently discarded medical certificate" was invented.** Mr Ace had re-filed the same dates _while testing_; the parent who obtains an MC after approval, the discarded evidence, the proposed attachment path and the school-policy question ("may they attach it, or is that contact-the-office?") were all built on top of the observed 200 to explain why it mattered. He ended it in one line — _"if the filing of the declaration is already approved why would the parent redeclare the same date bruh?"_ — and the fix collapsed to a 409. **Nobody reported it. Do not rebuild it.** **(2) "The API does not enforce with-medical ⇒ attach something, the portal does" is false.** It is enforced in `fileDeclarationSchema`'s `superRefine` **and** by `student_declarations_medical_needs_evidence_chk`. The portal enforces it too (Mr Ace), so all three agree — but the server was never relying on the portal.

⚠ **The portal brief previously said to show the confirmation screen on `alreadyFiled`**; that path now returns 409. `docs/handoff/2026-08-27-parent-portal-declarations.md` carries the full response and marks what changed. Mr Ace, 2026-08-29: **"all good on parent portal side."**
