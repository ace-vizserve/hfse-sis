...
- **Server Actions for some WRITE actions.** Raised by Mr Ace on 2026-08-29. His position: traditional API routes are fine as the general architecture — the interest is that **specific write actions** could be Server Actions instead, for **speed**. ⚠ **There is no recorded decision behind the current architecture** — "server action" appears **zero times** across every key-decision file, every context doc and CLAUDE.md, so it was never weighed and rejected; it was defaulted into.
  ...
  **The shortlist is not all 88 write routes.** It is: internal-only, triggered from a client component in this app, and performed repeatedly in one sitting — marking a register, entering marks down a column. On a once-a-term config save the saving is invisible. Stacks with `useOptimistic`, which would make those writes feel instant before the round trip even lands.

  ⚠ **Some routes can never convert:** the parent and admissions portals call cross-origin with a Bearer token, and `/api/sis/students/auto-sync` is cron-triggered with `CRON_SECRET`. ⚠ **A convention layer has grown on top of route handlers** — `requireRole()`, `lib/audit/log-action.ts`, `revalidateTag`/`invalidateDrillTags`, JSON errors with machine codes, `410 Gone` tombstones — and a Server Action equivalent would have to rebuild all five. Separate project; not scoped, not costed.
