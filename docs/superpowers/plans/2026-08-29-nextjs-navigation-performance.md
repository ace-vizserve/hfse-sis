...
⚠ **The plan as a whole is therefore not 97%, and saying otherwise would be the same mistake the last pass made.** Phases 1 and 3 are. Phase 2 is a five-minute experiment whose result is either "keep it" or "revert it" — that _outcome_ is ~97% safe; the _build passing_ is a coin flip until run.
...
- [ ] Fallbacks use `Skeleton` from `@/components/ui/skeleton`, sized to roughly match what replaces them. A fallback much shorter than its content trades a blank wait for a layout jump.
