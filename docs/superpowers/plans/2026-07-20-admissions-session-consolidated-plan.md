...
**Discovered during execution — this task's real scope is larger than originally drafted.** The original file list below (Files, unmodified) undercounted the blast radius: a pre-dispatch investigation found **3 real, functional (not comment-only) call sites** that query the template tables directly or call `computeSubjectConfigGaps` against a template-sourced "should" list, none of which were in the original delete/modify list:

- `app/(sis)/sis/page.tsx` (**the SIS Hub itself**) — imports `listTemplateSubjectLevelOfferings` from the doomed `lib/sis/template/queries.ts`, plus `computeSubjectConfigGaps`/`SubjectConfigGap` from `lib/sis/subject-config-gaps.ts`; its `loadSubjectConfigGapsForHubUncached` function queries the template table directly and feeds the Hub's **attention feed**.
- `lib/sis/hub-attention.ts` — imports the `SubjectConfigGap` type and renders one attention row per gap (`"${gap.levelLabel} is missing ${n} subject${...} from Structure Defaults"`).
- `app/(sis)/sis/admin/subjects/page.tsx` — imports both `listTemplateSubjectLevelOfferings` and `computeSubjectConfigGaps`; renders a warning banner with a **"wholesale" branch whose only remedy is a link to `/sis/admin/template` + "click Apply template"** — a page and an action this same task deletes.
- `lib/sis/readiness.ts` — the `subject-weights` readiness step's `fetchSubjectWeights` function queries `template_subject_level_offerings` directly (a real runtime DB query, not a comment) and imports `computeSubjectConfigGaps`.

Additionally, **`components/sis/template-manager-client.tsx`** (imported only by the doomed `app/(sis)/sis/admin/template/page.tsx` — confirmed via grep, its only other 2 hits repo-wide are unrelated comments) was missing from the original delete list; `git rm -r app/(sis)/sis/admin/template` does not touch it since it physically lives under `components/sis/`, not under the deleted route folder.

**Design decision (made during execution, not user-directed — the only defensible option once the comparison source is gone):** `computeSubjectConfigGaps`'s whole reason to exist was comparing an AY's actual subject/level attachments against what Structure Defaults said they _should_ be. There is no replacement "should" source — inventing one (e.g. comparing against the full subject catalog) would silently change the signal's meaning and risk false positives no one asked for. The safe, conservative simplification: **the signal narrows from "which specific subjects are missing per level, per the template" to "which levels have literally zero subjects attached at all."** This is strictly narrower (fewer, not more, false positives) and needs no comparison source — it's directly computable from each AY's own live `subject_level_offerings`. Apply this **same simplified signal, from one shared pure function**, to all three consumers (readiness step, Hub attention row, admin/subjects banner) so they can't drift from each other (this file's own pre-existing comments already state this "never drift" principle, KD #124/#128 — honor it, don't reintroduce three independent copies of a predicate).

**Files (corrected):**

- Delete: `app/(sis)/sis/admin/template/` (page + client component)
- Delete: `components/sis/template-manager-client.tsx` (missing from the original list — confirmed sole consumer is the deleted page)
...
- **Modify (new): `lib/sis/subject-config-gaps.ts`** — replace `computeSubjectConfigGaps(levels, subjects, templateOfferings, actualOfferings): SubjectConfigGap[]` (where `SubjectConfigGap = {levelId, levelLabel, missingSubjectCodes: string[]}`) with a simpler pure function taking only `levels` + `actualOfferings` (drop the `subjects`/`templateOfferings` params entirely — no more "should" source), returning levels with zero rows in `actualOfferings`, e.g. `findEmptyLevels(levels: Array<{id, label}>, actualOfferings: Array<{level_id, subject_id}>): Array<{levelId, levelLabel}>` (no `missingSubjectCodes` — that data no longer exists). Rename the type accordingly (e.g. `EmptyLevelGap`). Update the file's header comment (currently describes the template comparison) to describe the new, simpler check. The implementer chooses exact naming within these constraints — not consequential to correctness, just keep it internally consistent across all 3 call sites below and the test file.
- **Modify (new): `__tests__/sis/subject-config-gaps.test.ts`** — rewrite its test cases for the new simplified function signature (fewer cases needed: no-empty-levels, some-empty, level-with-zero-offerings-anywhere-in-input, multi-level sorted-by-label — mirror the existing file's structure/style, just drop every case that exercised per-subject-code gap lists since that behavior is gone).
- **Modify (new): `lib/sis/readiness.ts`** — `fetchSubjectWeights` (~line 676) drops its `subjects`/`template_subject_level_offerings` queries entirely, keeping only the `subject_level_offerings` query (scoped to the AY); computes `levelsFullyConfigured` = count of `relevantLevels` NOT in the new function's empty-levels result (i.e., levels with ≥1 subject); `missingCount` = the empty-levels count (this field is currently unused by `resolveSubjectWeightsStep`'s body — verify that's still true, keep the field for interface stability regardless, don't remove it from the return type without checking every consumer first). Update the `import { computeSubjectConfigGaps }` line to the new function name. Reword the header comment block above `fetchSubjectWeights` (~line 652-675) and the `resolveSubjectWeightsStep` design-intent comment (~line 17-22) to drop every "against Structure Defaults"/"template" phrasing — describe the check as "does every level have at least one subject configured."
- **Modify (new): `lib/sis/year-setup.ts`** — the `checklistSummary` function's `case 'subject-weights':` branch (~line 104-112) currently says `"...are missing subjects from Structure Defaults — those subjects won't appear on report cards."` — reword to not reference Structure Defaults, e.g. `"${gap} level${gap === 1 ? '' : 's'} ${gap === 1 ? 'has' : 'have'} no subjects configured yet — configure them so grades and report cards have somewhere to go."` (keep it plain-English, no dev jargon, matching this file's existing tone; the exact wording is the implementer's call as long as it doesn't reference the deleted concept).
- **Modify (new): `lib/sis/hub-attention.ts`** — the `subjectConfigGaps` loop (~line 112-122) currently builds `text: "${gap.levelLabel} is missing ${n} subject${...} from Structure Defaults"` + `meta: gap.missingSubjectCodes.join(', ')`. Update to the new shape (no `missingSubjectCodes`): `text: "${gap.levelLabel} has no subjects configured yet"`, drop or repurpose `meta` (check `AttentionRow`'s type — if `meta` is optional, just omit it here rather than fabricating a value). Keep `href: '/sis/admin/subjects'` and `actionLabel: 'Fix'` unchanged — both remain valid post-simplification. Update the type import from `SubjectConfigGap` to the new type name, and the input parameter's doc-comment (~line 46-51) to drop the Structure Defaults reference.
- **Modify (new): `app/(sis)/sis/page.tsx`** (the Hub) — remove the `listTemplateSubjectLevelOfferings` import + call from `loadSubjectConfigGapsForHubUncached` (~line 300-315); it now only needs `levels` + `listSubjectLevelOfferings(ayId)` (or whatever the existing "actual offerings" query already used elsewhere is called — check `lib/sis/subjects/queries.ts` for the right existing function rather than writing a new query) to feed the new simplified function. Update the `computeSubjectConfigGaps`/`SubjectConfigGap` imports to the new names. The `unstable_cache` wrapper, its tag (`sis:${ayCode}`), and the `loadSubjectConfigGapsForHub` try/catch wrapper are otherwise unchanged.
- **Modify (new): `app/(sis)/sis/admin/subjects/page.tsx`** — remove the `listTemplateSubjectLevelOfferings` import + the `templateOfferings` fetch (part of the page's `Promise.all`, ~line 88-105); replace the `computeSubjectConfigGaps` call (~line 125-131) with the new simplified function (levels + actual `offerings` only — `subjects` param no longer needed here either). **Rewrite the banner JSX** (~line 196-260): delete the `wholesale`/`totalMissing > 20` branch and its "go to Structure Defaults, click Apply template" copy entirely (that remedy no longer exists) along with `MAX_CODES_PER_LEVEL`/`shownGaps`/per-subject-code list rendering; replace with a single flat presentation — one banner, shown whenever the new function returns ≥1 empty level, headline `"${emptyLevels.length} level${...} with no subjects configured"`, body listing the affected level labels (comma-joined, or a short `<ul>` mirroring this component's existing list styling) with plain guidance to attach subjects using the page's own existing attach-subject UI (already present elsewhere on this same page — don't invent a new action, just point at what's already there). Keep the same amber alert box visual treatment (`border-brand-amber/30 bg-brand-amber-light`, the `AlertTriangle` icon tile) — this is a content/logic simplification, not a visual redesign. Update the header comment above the gap computation (~line 118-123, currently "Structure Defaults is the 'what SHOULD be configured' reference...") accordingly.
- **Verify only (no code change expected), `lib/sis/subjects/queries.ts`:** its only hit is a comment analogy ("mirroring the lib/sis/subject-config-gaps.ts::computeSubjectConfigGaps pattern") — confirm this is still comment-only after the rename above (the comment can stay as historical context, or be trivially updated to the new function name; implementer's call, not required).

- [ ] **Step 1: Grep sweep before deleting**

Run: `rg "template_sections|template_subject_configs|template_subject_level_offerings|apply_template_to_ay" --type ts --type tsx -l` **and separately** `rg "computeSubjectConfigGaps|SubjectConfigGap|listTemplateSubjectLevelOfferings|template-manager-client" --type ts --type tsx -l`.
Expected after both greps, once every Modify above is applied: **zero hits anywhere** except inside `docs/superpowers/` planning prose and the (deliberately kept, per KD #96's dormant-object precedent) historical `template.*` `AuditAction` enum members / humanizer labels in `lib/audit/log-action.ts`/`lib/audit/humanize.ts` if any exist (check — if there ARE dormant `AuditAction` members like `'section.realphabetize'`-style historical entries for template actions, leave them alone, same pattern as every other retired-feature audit trail in this codebase; do not invent new ones).

- [ ] **Step 2: Delete the files**

```bash
git rm -r "app/(sis)/sis/admin/template"
git rm components/sis/template-manager-client.tsx
...
- [ ] **Step 3b: Apply every "Modify (new)" item above** — `lib/sis/subject-config-gaps.ts` + its test, `lib/sis/readiness.ts`, `lib/sis/year-setup.ts`, `lib/sis/hub-attention.ts`, `app/(sis)/sis/page.tsx`, `app/(sis)/sis/admin/subjects/page.tsx`. Read each live file's current full relevant section before editing — none of the snippets above are verbatim-paste-ready (unlike most other tasks in this plan), they're descriptions of the required end state, because this scope was discovered during execution, not fully authored in advance. Re-run both greps from Step 1 after finishing to confirm the expected zero-hits state.
