...
7. **`subject_configs` is corrected, not just filled.** Only 1 row currently exists for all of AY2026 (should be ~1 per subject × level). `apply.sql` upserts (`ON CONFLICT ... DO UPDATE`, not `DO NOTHING`) the 16 rows this phase needs (8 subjects × {S1, S2}) with the real header-sourced weights/slots/max-scores.
...
- **Cross-term weight consistency check.** `subject_configs` weights are fixed per `(subject × level × AY)` (Hard Rule #1 / KD #4), not per-term — once T2's real grading sheets are available, verify T2's own headers agree with the weights this phase sets from T1. Any drift is a real signal (a genuine mid-year policy change, or a data-entry inconsistency) worth surfacing, not noise to ignore.
