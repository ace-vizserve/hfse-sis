...
The Academic Summary dashboard (`/records/academic-summary`, the relocated masterfile — KD #127/#122) is a monitoring surface, but most of its aggregates are dead-ends: only the "Standing out" watchlist links anywhere. A registrar verifying the masterfile sees "Grades recorded 280/312" or a donut "Gold 40" and can't get to _which_ students. This adds drill sheets to the high-value aggregates so a click reveals the names behind the number.
