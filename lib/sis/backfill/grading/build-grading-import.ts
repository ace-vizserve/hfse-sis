...
export interface BuildGradingImportResult {
  ...
  stats: {
    ...
    subjectLevelOfferingsWritten: number;
    sectionSubjectsWritten: number;
...
export function buildGradingImport(
  ...
  // subject_configs is now ONE row per subject for the whole AY (migration
  // 080 collapsed the old per-level rows — see the design doc's schema
  // note). Every sheet for the same subject MUST agree on weights/max-
  // scores, since there is no longer a column to represent a difference.
  // Hand-verified against the real T1 workbooks that Discipline 1 (S1) and
  // Integrity 1 (S2) agree for every one of the 8 subjects — this check is
  // a safety net for future terms, not an expected trigger today.
  const configsBySubject = new Map<string, ParsedSubjectSheet>();
  for (const sheet of sheets) {
    const existing = configsBySubject.get(sheet.subjectCode);
    if (!existing) {
      configsBySubject.set(sheet.subjectCode, sheet);
      continue;
    }
    const sameShape =
      existing.wwWeight === sheet.wwWeight &&
      existing.ptWeight === sheet.ptWeight &&
      existing.qaWeight === sheet.qaWeight &&
      existing.qaTotal === sheet.qaTotal &&
      arraysEqual(existing.wwTotals, sheet.wwTotals) &&
      arraysEqual(existing.ptTotals, sheet.ptTotals);
    if (!sameShape) {
      throw new Error(
        `buildGradingImport: subject ${sheet.subjectCode} has divergent weights/max-scores between ` +
          `${existing.levelCode} ${existing.sectionName} and ${sheet.levelCode} ${sheet.sectionName} — ` +
          'subject_configs now holds exactly one row per subject for the whole AY (migration 080), ' +
          'so this cannot be represented and must be resolved by hand before importing.'
      );
    }
  }

  // subject_level_offerings still tracks "this subject applies to this
  // level this year" — one row per distinct (subjectCode, levelCode) pair.
  const offeringKey = (subjectCode: string, levelCode: string) =>
    `${subjectCode}::${levelCode}`;
  const offeringsByKey = new Map<string, ParsedSubjectSheet>();
  for (const sheet of sheets) {
    offeringsByKey.set(offeringKey(sheet.subjectCode, sheet.levelCode), sheet);
  }

  const stats: BuildGradingImportResult['stats'] = {
    subjectConfigsWritten: configsBySubject.size,
    subjectLevelOfferingsWritten: offeringsByKey.size,
    sectionSubjectsWritten: sheets.length,
...
function buildPreviewSql(
  ...
  lines.push('--');
  lines.push(
    `-- subjectConfigs=${stats.subjectConfigsWritten} subjectLevelOfferings=${stats.subjectLevelOfferingsWritten} sectionSubjects=${stats.sectionSubjectsWritten} gradingSheets=${stats.gradingSheetsWritten} gradeEntries=${stats.gradeEntriesWritten}`
  );
  lines.push('--');
  lines.push(
...
function buildApplySql(
  ...
  configsBySubject: Map<string, ParsedSubjectSheet>,
  offeringsByKey: Map<string, ParsedSubjectSheet>
): string {
  const lines: string[] = [];
  ...
  lines.push('--');
  lines.push(
    '-- subject_configs is one row per subject for the whole AY (migration'
  );
  lines.push(
    '-- 080 collapsed the old per-level rows) — subject_level_offerings and'
  );
  lines.push(
    '-- section_subjects carry the level/section eligibility that used to'
  );
  lines.push('-- live on subject_configs itself.');
  lines.push('--');
  ...
  // --- 1) subject_configs — one row per subject, no level dimension ---
  lines.push('drop table if exists _ay26grd_subject_configs;');
  lines.push(
    'create temp table _ay26grd_subject_configs (subject_code, ww_weight, pt_weight, qa_weight, ww_max_slots, pt_max_slots, qa_max) as'
  );
  lines.push('values');
  const configRows = [...configsBySubject.values()].map(
    (s) =>
      `  (${sqlString(s.subjectCode)}, ${s.wwWeight}, ${s.ptWeight}, ${s.qaWeight}, ${s.wwTotals.length}, ${s.ptTotals.length}, ${s.qaTotal ?? 'null'})`
  );
  lines.push(configRows.join(',\n') + ';');
  lines.push('');
  lines.push(
    'insert into subject_configs (academic_year_id, subject_id, ww_weight, pt_weight, qa_weight, ww_max_slots, pt_max_slots, qa_max)'
  );
  lines.push(
    'select ay.id, sub.id, c.ww_weight, c.pt_weight, c.qa_weight, c.ww_max_slots, c.pt_max_slots, c.qa_max'
  );
  ...
  // --- 2) subject_level_offerings — "this subject applies to this level
  // this year" (additive only, never removes an existing offering) ---
  lines.push('drop table if exists _ay26grd_offerings;');
  lines.push(
    'create temp table _ay26grd_offerings (subject_code, level_code) as'
  );
  lines.push('values');
  const offeringRows = [...offeringsByKey.values()].map(
    (s) => `  (${sqlString(s.subjectCode)}, ${sqlString(s.levelCode)})`
  );
  ...
  lines.push('select ay.id, sub.id, lvl.id');
  lines.push('from _ay26grd_offerings o');
  lines.push(`join academic_years ay on ay.ay_code = ${sqlString(ayCode)}`);
  lines.push('join subjects sub on sub.code = o.subject_code');
  lines.push('join levels lvl on lvl.code = o.level_code');
  lines.push(
    'on conflict (subject_id, level_id, academic_year_id) do nothing;'
  );
  lines.push('');

  // --- 3) grading_sheets (also stages section_subjects rows) ---
  ...
  // --- 3a) section_subjects — links each section to the subject_configs
  // row it uses; resolved via subject_id + academic_year_id only, no
  // level_id (matches migration 080's sync_section_subjects_for_ay). ---
  lines.push('insert into section_subjects (section_id, subject_config_id)');
  lines.push('select distinct sec.id, sc.id');
  ...
  lines.push('on conflict (section_id, subject_config_id) do nothing;');
  lines.push('');
  ...
  lines.push(
    `select count(*) as subject_level_offerings_rows from subject_level_offerings slo join academic_years ay on ay.id=slo.academic_year_id where ay.ay_code=${sqlString(ayCode)};`
  );
  lines.push(
    `select count(*) as section_subjects_rows from section_subjects ss join sections sec on sec.id=ss.section_id join academic_years ay on ay.id=sec.academic_year_id where ay.ay_code=${sqlString(ayCode)};`
  );
  lines.push(
