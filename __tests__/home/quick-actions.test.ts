...
// A switch is a real role change now, so a teaching admin working as a teacher
// arrives here as `'teacher'` and gets the teacher table — the same rows, the
// same profile filter and the same fallback a plain teacher gets, all of which
// the cases above already pin.
describe('getQuickActions — one role picks the rows and checks them', () => {
  it('every row it offers is one the same role may open', () => {
    for (const [role, profile] of [
      ...
      for (const action of getQuickActions(role, [], profile, CAPS_OF(role))) {
        expect(
          isRouteAllowed(hrefPathname(action.href), role),
          `${action.label} points at ${action.href}, which the proxy refuses a ${role}`
        ).toBe(true);
      }
    }
  });
