-- supabase/migrations/075_enrolment_timestamp.sql
--
-- Adds a write-once enrolment timestamp so the actual moment a student became
-- 'Enrolled' / 'Enrolled (Conditional)' is captured from go-live. The funnel
-- currently never records this (the *UpdatedDate columns are date-only and get
-- overwritten on every later edit), so "time to enrol" / cohort-by-enrolment
-- analytics had nothing to read (0/490 in prod).
--
--   ay{YYYY}_enrolment_status."enrolledAt"  timestamptz nullable (write-once)
--
-- The route (app/api/sis/students/[enroleeNumber]/stage/[stageKey]) stamps
-- enrolledAt = now() ONLY when it is currently NULL, on every path that reaches
-- Enrolled — so a re-enrol or a later edit never overwrites the original moment.
--
-- This migration (1) heals every existing ay####_enrolment_status table in
-- place (ADD COLUMN IF NOT EXISTS — idempotent), and (2) re-emits
-- create_ay_admissions_tables() from its NEWEST live body (migration 069, NOT
-- a stale copy — KD #119 hazard) with "enrolledAt" added to the status block so
-- every future AY ships with the column.
--
-- Idempotent + safe to re-run. Apply after 074.
--
-- DEPLOY ORDERING: the stage route references "enrolledAt", so this migration
-- MUST be applied to the DB before that code deploys (one shared project per
-- KD #1, so a single apply covers test + prod). Same ordering rule as KD #144.

-- =====================================================================
-- 1. Heal existing enrolment_status tables (idempotent ADD COLUMN).
--    Covers every AY (prod + the test AYs). NULL on existing rows is
--    correct — historical enrolments have no captured moment, and the
--    write-once route only stamps from now on.
-- =====================================================================
do $$
declare
  r record;
begin
  ...
  loop
    execute format(
      'alter table public.%I add column if not exists "enrolledAt" timestamptz null',
      r.tablename
    );
  end loop;
end $$;
...
-- =====================================================================
-- 2. Re-emit create_ay_admissions_tables — migration 069 body (the newest
--    live definition: applications keeps "stpApplicationStatus" + CHECK,
--    status keeps the terminal-reason columns) with the new status
--    "enrolledAt" column added. Everything else is migration 069 verbatim.
-- =====================================================================
...
begin
  ...
  -- status CHECK (restored in 069). Verbatim from migration 069.
  execute format($ddl$
  ...
  -- ay{YYYY}_enrolment_status — migration 069 body + the new write-once
  -- "enrolledAt" timestamptz column (the one part 075 adds).
  execute format($ddl$
    create table if not exists public.%I (
      id bigint generated by default as identity not null,
  ...
  -- ay{YYYY}_enrolment_documents — migration 069 verbatim.
  execute format($ddl$
    create table if not exists public.%I (
      id bigint generated by default as identity not null,
      created_at timestamp with time zone null default (now() at time zone 'Asia/Singapore'::text),
  ...
  -- ay{YYYY}_discount_codes — migration 069 verbatim.
  execute format($ddl$
    create table if not exists public.%I (
      id bigint generated by default as identity not null,
      created_at timestamp with time zone not null default now(),
  ...
  -- Enable RLS + canonical permissive policy on each table (069 verbatim).
...
comment on function public.create_ay_admissions_tables is
  'Creates the 4-table admissions set for a new AY. Applications carries stpApplicationType (026) + stpApplicationStatus (050, restored 069); status carries applicationTerminalReason/Notes (067) + write-once enrolledAt (075). KD #61/#96/#111/#119.';
