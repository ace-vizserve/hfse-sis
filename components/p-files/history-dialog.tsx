...
export function HistoryDialog({
  ...
  // queryFn forwards the abort signal so a fast close aborts the request, and
  // the route's bespoke `body.error` message surfaces via ApiError.message in
  // the inline error state below.
  ...
  const loading = revisionsQuery.isLoading;
  const revisions = revisionsQuery.data ?? null;
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-xl!">
        ...
        <ScrollArea className="h-[60vh] py-3">
          ...
          {!loading && revisionsQuery.isError && (
                    ...
                    <li key={rev.id} className="flex flex-col gap-2 px-4 py-3">
                      ...
                      {isParentPortal && (
                        <p className="rounded-md border border-brand-amber/30 bg-brand-amber-light/30 px-3 py-2 text-[11px] leading-relaxed text-muted-foreground">
                          ...
                          Metadata (status, expiry, who, when) is preserved
                          here.
                        </p>
                      )}
