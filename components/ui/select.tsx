...
>(({ className, ...props }, ref) => (
  <SelectPrimitive.Label
    ref={ref}
    className={cn(
      'px-2 py-1.5 font-mono text-[10px] font-semibold uppercase tracking-[0.14em] text-ink-4',
      className,
    )}
    {...props}
  />
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ...
    className={cn(
      ...
      'data-[state=checked]:font-medium data-[state=checked]:text-brand-indigo',
  ...
  >
    <span className="absolute left-2 flex h-3.5 w-3.5 items-center justify-center">
      <SelectPrimitive.ItemIndicator>
        <Check className="h-4 w-4 text-brand-indigo" />
      </SelectPrimitive.ItemIndicator>
    </span>
