...
export function HubReadinessPopover({ readiness }: { readiness: AyReadiness }) {
  ...
  return (
    <Popover>
      ...
      <PopoverContent align="end" className="w-max p-0">
        <ScrollArea className="h-96 w-full">
          <div className="flex items-center justify-between border-b border-border px-4 py-3">
            ...
            <Link
              href="/sis/ay-setup"
              className="flex items-center gap-1 text-[12px] font-semibold text-brand-indigo-deep hover:underline"
            >
              Full checklist <ArrowUpRight className="size-4" />
            </Link>
          </div>
          {groups.map((group) => (
            <div key={group.label}>
